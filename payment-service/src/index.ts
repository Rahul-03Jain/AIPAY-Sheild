...
app.get("/transaction-status/:id", async (req, res) => {
  ...
  try {
    const result = await client.query(
      `SELECT id as transaction_id, status, fraud_score, fraud_risk_level,
              fraud_decision, original_amount, original_currency,
              converted_amount, converted_currency, payment_method,
...
app.get("/user-transactions", async (req, res) => {
  ...
  try {
    const result = await client.query(
      `SELECT id as transaction_id, status, original_amount, original_currency,
              converted_amount, converted_currency, payment_method, created_at
       FROM transactions
       WHERE user_id=$1
